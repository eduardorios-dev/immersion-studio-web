...
import { ArrowDown, ArrowRight, BriefcaseBusiness, Building2, CalendarDays, Camera, Check, ClipboardList, CloudUpload, Globe2, Handshake, Hotel, House, ImageIcon, Layers3, LayoutTemplate, MapPin, MapPinned, Menu, MonitorSmartphone, Puzzle, ScanLine, SlidersHorizontal, Sparkles, Star, TabletSmartphone, Users, X } from 'lucide-react';
import './styles.css';
...
function App(){
  ...
  return <>
    ...
    <main id="top">
      <section className="hero hero-editorial dark">
        <span className="immersive-mark hero-immersive-mark" aria-hidden="true">360°</span>
      ...
      <section className="intro section" id="soluciones"><div className="section-label">01 / QUÉ HACEMOS</div><div className="intro-layout"><h2>Experiencias virtuales que transforman la forma de presentar tus espacios.</h2><div className="intro-content"><div className="intro-notes"><p>Integramos navegación, fotografía, video, información y contenido multimedia en una sola experiencia digital.</p><p>Cada proyecto se diseña para comunicar mejor el valor de una propiedad y hacer que explorarla sea claro, visual y memorable.</p></div><figure className="intro-photo"><img src="/intro-recorrido-clean.jpg" alt="Habitación Master de Casa Loreto 1893"/><span className="immersive-mark" aria-hidden="true">360°</span></figure></div></div><div className="service-grid"><article><span>01</span><h3>Recorridos 360°</h3><p>Tours inmersivos que permiten explorar cada rincón con libertad y realismo.</p></article><article><span>02</span><h3>Plataformas interactivas</h3><p>Entornos digitales intuitivos que centralizan contenidos y facilitan la toma de decisiones.</p></article><article><span>03</span><h3>Experiencias personalizadas</h3><p>Soluciones a la medida de cada proyecto, diseñadas para conectar y dejar huella.</p></article></div></section>

      <section className="steps dark section"><div className="section-label">02 / CÓMO FUNCIONA</div><div className="steps-title"><p>Tus futuros clientes:</p><h2>Exploran.<br/>Conocen.<br/><em>Deciden.</em></h2></div><div className="step-list"><article><b>01</b><div><h3>Exploran</h3><p>Recorren los espacios libremente desde cualquier dispositivo.</p></div></article><article><b>02</b><div><h3>Conocen</h3><p>Descubren áreas, servicios, amenidades y puntos relevantes dentro de una misma experiencia.</p></div></article><article><b>03</b><div><h3>Deciden</h3><p>Acceden a mayor contexto antes de reservar, visitar o continuar un proceso comercial.</p></div></article></div></section>

        <section className="platform section"><div className="section-label">03 / LA PLATAFORMA</div><div className="platform-grid"><div><h2>Todo el contenido en una sola experiencia.</h2><p className="big-copy">Un entorno intuitivo que conecta recorridos 360°, fotografías, video, información y navegación entre diferentes áreas.</p><ul className="check-list"><li><Check/> Experiencia adaptada a tu identidad</li><li><Check/> Contenido organizado y fácil de actualizar</li><li><Check/> Acceso directo o integración en tu sitio</li></ul></div><figure className="catalog-device-composition"><img src="/plataforma-dispositivos-aprobada.png" alt="Plataforma de Bosques de Monterreal adaptada a laptop, tablet y celular"/></figure></div></section>
      ...
      <section className="device-band"><div><Globe2/><h3>Web</h3><p>Enlace directo o integrado en tu sitio.</p></div><div><TabletSmartphone/><h3>Móvil y tablet</h3><p>Una experiencia adaptada a cada pantalla.</p></div><div><MonitorSmartphone/><h3>Realidad virtual</h3><p>Presentaciones, exposiciones y usos inmersivos.</p></div></section>
        ...
        <section className="applications section" id="aplicaciones"><div className="section-label">04 / APLICACIONES</div><div className="split-head"><h2>Una experiencia para cada tipo de espacio.</h2><p className="bullet-copy">Soluciones flexibles para hospitality, eventos, real estate y proyectos de mayor escala.</p></div><div className="sector-grid">{sectors.map((s,i)=>{const Icon=s[2];return <article key={s[0]}><span>{String(i+1).padStart(2,'0')}</span><Icon/><h3>{s[0]}</h3><p>{s[1]}</p></article>})}</div></section>

        <section className="projects dark section" id="proyectos"><div className="section-label">05 / PROYECTOS</div><div className="split-head projects-head"><h2>Proyecto destacado.</h2><a className="button ghost" href="/proyectos">Ver proyectos <ArrowRight/></a></div><div className="project-card"><div className="project-art"><img src="/casa-loreto.jpg" alt="Plataforma interactiva de Casa Loreto 1893"/></div><div className="project-info"><small>HOTEL BOUTIQUE · MÉRIDA, YUCATÁN</small><h3>Casa Loreto 1893</h3><span>PLATAFORMA INTERACTIVA</span><p>Experiencia virtual para presentar habitaciones, espacios comunes y la identidad de la propiedad.</p><a className="text-link" href="https://casaloreto1893.netlify.app/" target="_blank" rel="noreferrer">Ver experiencia 360° <ArrowRight/></a></div></div></section>

      <section className="pricing section" id="planes"><div className="section-label">06 / PLANES</div><div className="split-head"><h2>Una solución para cada proyecto.</h2><p className="bullet-copy">Elige un punto de partida. Cada experiencia puede adaptarse a tus espacios, contenidos y objetivos.</p></div><div className="plans">{plans.map(p=><article className={p.featured?'featured':''} key={p.name}><small>{p.tag}</small><h3>{p.name}</h3><p>{p.intro}</p><ul>{p.items.map(x=><li key={x}><Check/>{x}</li>)}</ul><a href="#contacto">Solicitar cotización <ArrowRight/></a></article>)}</div><p className="custom">¿Necesitas algo diferente? <b>Desarrollamos soluciones personalizadas según el proyecto.</b></p></section>

      <section className="commercial commercial-catalog dark section"><div className="commercial-overlay"/><div className="commercial-content"><div className="section-label">07 / USOS COMERCIALES & VR</div><h2>Lleva tu propiedad a exposiciones, reuniones y presentaciones.</h2><p>Permite que clientes, compradores e inversionistas recorran el espacio mediante realidad virtual, incluso cuando la propiedad está lejos o no puede visitarse en ese momento.</p><div className="benefit-list"><div><MapPin/><span>Presenta propiedades ubicadas en otra ciudad.</span></div><div><Users/><span>Atrae más atención en exposiciones y stands.</span></div><div><Handshake/><span>Facilita demostraciones para compradores e inversionistas.</span></div><div><LayoutTemplate/><span>Explica espacios amplios de una forma visual.</span></div><div><Star/><span>Crea una experiencia más memorable.</span></div></div><strong>Tu propiedad puede estar presente, aunque se encuentre a kilómetros de distancia.</strong></div></section>

      <section className="process section"><div className="section-label">08 / PROCESO</div><div className="process-head"><h2>Nos encargamos de todo el proceso.</h2><p className="bullet-copy">Desde la planeación hasta la publicación, coordinamos cada etapa para entregar una experiencia lista para compartir.</p></div><div className="process-line">{[
        ['Planeación','Definimos alcance, espacios y objetivos.',ClipboardList],
        ['Captura 360°','Registramos cada área con equipo especializado.',ScanLine],
        ['Postproducción','Editamos imagen, color y navegación.',SlidersHorizontal],
        ['Diseño de experiencia','Organizamos contenido y recorrido visual.',LayoutTemplate],
        ['Integración','Conectamos fotografía, video e información.',Puzzle],
        ['Publicación y entrega','Publicamos, revisamos y entregamos el proyecto.',CloudUpload]
      ].map(([title,description,Icon],i)=><div key={title}><span>{String(i+1).padStart(2,'0')}</span><Icon/><h3>{title}</h3><p>{description}</p></div>)}</div><p className="process-close bullet-copy">Un proceso coordinado. Una experiencia lista para presentar.</p></section>
